...
Next booloader sets up "some useful defaults" as docstring says:
...
Merb.dir_for is a reader for registred component paths.
...
It's a good way to do all kind of things befor app loads: from plugins configuration
to confitional requires (in case env-specific init files do not satisfy your needs).
...
Merb::NootLoader::LoadClasses (lib/merb-core/bootloader.rb):
<pre>
