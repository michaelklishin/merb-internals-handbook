...
and accordingly to action specified on command like Merb instance is either killed or started.

Merb.start can be found in lib/merb-core.rb in Merb Git repository.
...
means that if you set Merb port, daemonization options or pid file location in config file
of your application, it would have no effect on application start.
So you must specify port, daemonization option and pid file location from command
line or in the startup script you may write instead of default merb executable.
