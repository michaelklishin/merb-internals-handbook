...
something else, then assigning deserialized session object to request instance.
...
Merb::SessionContainer is a base class and supposed to be subclassed.
<pre>
<code>
module Merb
...
cookie session. It's also base64 encrypted for integrify (and not
for security through obscurity). Cookie session store uses
OpenSSL HMAC and SHA1 algorithms for digest and is most likely
to perfom worse than memory, memcached or database sessions.
...
Generation of Set-Cokie header is called extraction in Merb and happens once
