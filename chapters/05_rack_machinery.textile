...
conditional get support to csrf attacks protected to session setup and finalization.
...
Here is how Thin adapter looks like:
...
What it does is setting up Rack applications cascade web server is going to use.
...
port, processes requests using middleware cascade that may (or may not) call Merb Rack application stored in Merb::Config[:app].
...
Most of the time end users do not implement their own middleware cascades and
Merb uses default stack: path prefix and static files middleware.